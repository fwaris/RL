...
module DQNModel =
    ...
    let create (fmodel: unit -> IModel) = 
        let tgt : IModel  = fmodel()
        let onln : IModel = fmodel()
        tgt.Module.parameters() |> Seq.iter (fun p -> p.requires_grad <- false)
        let models = {Target=tgt; Online=onln}
...
module DQN =
    ...
    let private randomChoice (choices:int list) =
        match choices with
        | [] -> failwith "randomChoice requires at least one valid action"
        | _ ->
            let idx = randint choices.Length
            choices.[idx]
    ...
    let selectActionMasked (state:torch.Tensor) ddqn step (validActions:int list) =
        ...
        | _ ->
            if rand() < step.ExplorationRate then
                randomChoice validActions,true
            else
                maskedBestAction state ddqn validActions
